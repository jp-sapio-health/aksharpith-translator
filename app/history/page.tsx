...
type SortKey = 'date-desc' | 'date-asc' | 'words-desc' | 'flags-desc';
...
export default function HistoryPage() {
  ...
  // Search filters BEFORE grouping; sort applies AFTER grouping.
  ...
  const items = (() => {
    const grouped = groupTranslations(filteredTranslations);
    const itemDate = (i: HistoryItem) =>
      i.type === 'book' ? i.book.createdAt : i.translation.createdAt;
    const itemWords = (i: HistoryItem) =>
      i.type === 'book' ? i.book.totalWords : i.translation.outputWordCount;
    const itemFlags = (i: HistoryItem) =>
      i.type === 'book' ? i.book.totalFlags : (i.translation.flagsCount ?? 0);
    const sorted = [...grouped];
    switch (sortKey) {
      case 'date-asc':
        sorted.sort((a, b) => itemDate(a).localeCompare(itemDate(b)));
        break;
      case 'words-desc':
        sorted.sort((a, b) => itemWords(b) - itemWords(a));
        break;
      case 'flags-desc':
        sorted.sort((a, b) => itemFlags(b) - itemFlags(a));
        break;
      case 'date-desc':
      default:
        sorted.sort((a, b) => itemDate(b).localeCompare(itemDate(a)));
        break;
    }
    return sorted;
  })();
          ...
          <div className="flex flex-col gap-2">
            <div className="flex flex-col sm:flex-row gap-2 mb-2">
              <input
                ...
                placeholder="Search title, book, content…"
                className="flex-1 rounded-md border bg-paper px-3 py-2 text-sm font-mono text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-foreground/10"
              />
              <select
                value={sortKey}
                onChange={(e) => setSortKey(e.target.value as SortKey)}
                className="rounded-md border bg-paper px-3 py-2 text-xs font-mono uppercase tracking-wider text-foreground focus:outline-none focus:ring-2 focus:ring-foreground/10"
              >
            ...
            {items.length === 0 && (
              <div className="text-center py-8 rounded-md border bg-paper">
            ...
            <div className="text-xs text-muted-foreground mb-1 tabular-nums">
              ...
              {search.trim() && (
                <> matching <span className="font-mono text-foreground">{search.trim()}</span></>
              )}
              {' · '}
